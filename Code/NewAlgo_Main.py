...
for i in range(0, number_of_clients):
    ...
    server.Alpha_matrix[:,i]=np.ones(number_of_clients) # initial omega values
...
#        C_Gs_van=None
        ...
        # <new algorithm>
        omega_matrix = []
        omegaeta = 1/math.sqrt(number_of_iterations) # eta in the exponent
        # omegaeta = rate
        for i in range(0, number_of_clients):  # updating omega_ij
            prev_omega = server.Alpha_matrix[:,i]
            new_omega = []
            for j in range(0, number_of_clients):
                # new_omega.append(prev_omega[j]*np.exp(-omegaeta*((server.Losses[i][j])**2)/2))
                new_omega.append(prev_omega[j]*np.exp(-omegaeta*server.Losses[i][j]))
            omega_matrix.append(new_omega)
        
        # updating individual neural network weights using model.fit
        
        G=np.zeros((number_of_clients,len(server.cl[0].vector_weights)))
        for i in range(0, number_of_clients):
            server.cl[i].model.fit(x=cl[i].x_batch,y=cl[i].y_batch, epochs=10,batch_size=1,workers=4,use_multiprocessing=True,verbose=False)
            print(len(cl[i].x_batch))
            G[i,:] = np.squeeze(server.cl[i].vector_weights)
        if server.number_of_clients==1:
            G=[np.expand_dims(G,axis=1)]
        else:
            G=[np.expand_dims(temp,axis=1) for temp in G]
        old_nn_wts=G
        new_nn_wts=G
        
        # updating individual neural network weights using one step of SGD
        """
        w_gradient = hf.get_N_grad(server)
        learning_rate=rate
        if optimiser=='momentum':
            new_indiv_wts=hf.grad_des_asc(server.Weights,learning_rate,lr,-1)
        else:
            new_indiv_wts=hf.grad_des_asc(server.Weights,w_gradient,learning_rate,-1)
        w_gradient=None
        learning_rate=None
        gc.collect()
        if server.number_of_clients==1:
            new_indiv_wts=[np.expand_dims(new_indiv_wts,axis=1)]
        else:
            new_indiv_wts=[np.expand_dims(temp,axis=1) for temp in new_indiv_wts]
        old_nn_wts=new_indiv_wts
        new_nn_wts=new_indiv_wts
        """
        # if not using updated individual neural network weights using one step of SGD, uncomment below
        """
        old_nn_wts=server.Weights
        new_nn_wts=server.Weights
        """
        for i in range(0, number_of_clients): # updating neural network weights using sum_j(omega_ij*theta_ij)
            temp = omega_matrix[i][0]*old_nn_wts[0]
            for j in range(1, number_of_clients):
                temp+=omega_matrix[i][j]*old_nn_wts[j]
            temp=temp/np.sum(omega_matrix[i]) # normalisation
            new_nn_wts[i]=temp
        server.Weights = new_nn_wts #New Query Weights
        server.finalise_client_weights(server.Weights) # loading neural netwoek with updated weights
        [C_Gs, C_Ls]=hf.Query_Clients(server,server.Weights,False) # computing updated losses
        server.Losses=C_Ls
        server.grad_W=C_Gs
        gc.collect()
        for i in range(0, number_of_clients): # setting previous omeha_ij to the current ones
            server.Alpha_matrix[:,i]=omega_matrix[i]
        new_nn_weights = None
        # </new algorithm>
        """
        # for i in range(0, number_of_clients):
...
#        Loss_History.append(-1)
        ...
        if iterations%5==0: #Save Plots Every 5 Iterations
            hf.Graph_Me(programID,0,iterations, btchsze)
        loops+=1
        print("\n")
