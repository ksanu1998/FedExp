...
def get_N_grad(server):
    ...
    for l in range(num_clients):
        alpha= np.eye(num_clients)[:,l]
        lamb=np.ones(num_clients)/num_clients
...
def Graph_Me(loading_ID,start,iterations, btchsze):
    ...
    THINGS_TO_PLOT=[
        "avg_acc_",
        f"Local_Training_acc_batch_{btchsze}_",
        f"Local_Testing_acc_batch_{btchsze}_",
        "avg_acc_vanilla_"
        ]
    NAMES=[
        'Train Accuracy New Algorithm',
        'Test Accuracy New Algorithm',
...
#                plt.ylabel("Accuracy %",fontsize=22)
                ...
                plt.ylabel("Accuracy %")
                plt.title('Average Accuracies (MNIST)',fontsize=22)
...
#%                                              Plotting Losses and WEIGHTS
    ...
    NAMES=[
        ...
        'Norm Squared Difference of Weights Averaged (New Algorithm)',
